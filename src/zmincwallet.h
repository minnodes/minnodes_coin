...
#ifndef Minnodes_ZMINCWALLET_H
#define Minnodes_ZMINCWALLET_H
...
public:
    CzMINCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMINC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMINC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Minnodes_ZMINCWALLET_H
