...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MINC);
    ...
    if (!settings.contains("nAnonymizeMinnodesAmount"))
        settings.setValue("nAnonymizeMinnodesAmount", 1000);

    nAnonymizeMinnodesAmount = settings.value("nAnonymizeMinnodesAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMinnodesAmount"))
        SoftSetArg("-anonymizeminnodesamount", settings.value("nAnonymizeMinnodesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in minnodes.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMinnodesAmount:
            return QVariant(nAnonymizeMinnodesAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMinnodesAmount:
            nAnonymizeMinnodesAmount = value.toInt();
            settings.setValue("nAnonymizeMinnodesAmount", nAnonymizeMinnodesAmount);
            emit anonymizeMinnodesAmountChanged(nAnonymizeMinnodesAmount);
            break;
        case CoinControlFeatures:
