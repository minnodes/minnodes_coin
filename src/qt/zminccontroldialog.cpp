...
#include "ui_zminccontroldialog.h"
...
std::set<std::string> zMINCControlDialog::setSelectedMints;
std::set<CMintMeta> zMINCControlDialog::setMints;

zMINCControlDialog::zMINCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zMINCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zMINCControlDialog::~zMINCControlDialog()
{
    delete ui;
}

void zMINCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zMINCControlDialog::updateList()
{
...
void zMINCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zMINCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzMINC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzMINCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zMINCControlDialog::GetSelectedMints()
{
...
void zMINCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
