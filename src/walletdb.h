...
public:
    ...
    bool WriteZMINCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMINCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMINCSeed_deprecated(uint256& seed);
    bool EraseZMINCSeed();
    bool EraseZMINCSeed_deprecated();

    bool WriteZMINCCount(const uint32_t& nCount);
    bool ReadZMINCCount(uint32_t& nCount);
