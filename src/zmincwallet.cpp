...
CzMINCWallet::CzMINCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zminc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMINCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMINCSeed_deprecated()) {
                    LogPrintf("%s: Updated zMINC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zminc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zminc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMINCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMINCCount(nCountLastUsed);
    else if (!walletdb.ReadZMINCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMINCWallet::Lock()
{
    seedMaster = 0;
}

void CzMINCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMINCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMINC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMINCWallet::LoadMintPoolFromDB()
{
...
void CzMINCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMINCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMINCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmincTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMINCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMINC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmincTracker which also adds to database
    pwalletMain->zmincTracker->Add(dMint, true);
...
void CzMINCWallet::SeedToZMINC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMINCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMINCWallet::GenerateDeterministicZMINC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMINCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMINCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
