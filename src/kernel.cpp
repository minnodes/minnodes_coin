...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzMINCStake(spend));
    } else {
        ...
        CMincStake* mincInput = new CMincStake();
        mincInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(mincInput);
    }
