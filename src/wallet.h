...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MINC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMINC send
enum ZerocoinSpendStatus {
    ZMINC_SPEND_OKAY = 0,                            // No error
    ZMINC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMINC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMINC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMINC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMINC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMINC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMINC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMINC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMINC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMINC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMINC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMINC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMINC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMINC_SPENT_USED_ZMINC = 14,                      // Coin has already been spend
    ZMINC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMINC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zMINCBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMINCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmincAuto) const;
    ...
    */
    ...
    CzMINCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMINCTracker> zmincTracker;
    ...
    void setZWallet(CzMINCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmincTracker = std::unique_ptr<CzMINCTracker>(new CzMINCTracker(strWalletFile));
    }

    CzMINCWallet* getZWallet() { return zwalletMain; }
    ...
    void setzMINCAutoBackups(bool fEnabled)
    {
    ...
    /** zMINC reset */
    boost::signals2::signal<void()> NotifyzMINCReset;
